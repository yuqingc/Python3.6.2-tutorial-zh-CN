...
除了索引之外，也支持 *切分 (slicing)* 字符串。字符串索引用来或许某个字符个体，而 切分 允许你获取 子字符串：
```python
...
因为索引值有很有用的默认属性。省略的第一个索引值缺省为 0，省略的第二个索引值就会缺省为被切分字符串的长度。
...
一个记住切分是如何工作的方法是，想象一个位于字符串之间的索引，这个索引的第一个字符左边缘是 0。然后最后一个字符 *n* 的右边缘具有索引 *n*。举个栗子：
```python
+---+---+---+---+---+---+
 | P | y | t | h | o | n |
 +---+---+---+---+---+---+
...
第一行数字显示了在字符串中从0~6的位置。第二行显示了倒数的位置。从 *i* 到 *j* 的切分组成了所有位于标签 *i* 和 *j* 之间的所有字符。

对于非倒数的索引值来说，如果两个索引值都在字符串的范围之内，切分的子字符串的长度就是两个索引值之差。例如，`word[1:3]` 的长度就是2.
...
Python 可以识别大量的复合数据结构，一般用来把其他的数值组合在一起。用途最广的就是 *列表*，它可以写成在一对方括号中由逗号隔开的一个列表。列表可能包含不同类型的数据，但是通常所有的项目 (item, 别的资料可能会翻译成“元素”，你知道是一个意思就可以了) 都是同一个类型的。
...
就像字符串 (以及其他所有内建队列类型) 一样，列表可以被索引和切分：

```python
>>> squares[0]  # 索引一个列表会返回一个项目
1
>>> squares[-1]
25
>>> squares[-3:]  # 切分操作返回一个新的列表
[9, 16, 25]
```

所有的切分 (slice) 操作都会返回一个包含所要求的项目的新列表。这就意味着下面的切分操作会返回原来列表的一个新的副本 (浅复制)：
...
你也可以通过使用 append() 方法，在列表的末尾加上新的项目。(我们在后面江湖看到更多关于方法的内容)
...
切分的时候赋值也是可以的，这甚至可以改变列表的长度或者把他完全清空：

...
小的匿名函数可以通过关键字 `lambda` 来创建。这个函数返回他的两个参数之和：`lambda a, b: a+b`。Lambda 函数可以在需要函数对象的地方使用。它们在语法上限制为一个表达式。从语义上说，它们仅仅是一个普通函数定义的语法糖。就像嵌套定义函数一样，lambda 函数可以从包含它的范围中引用变量。
